  ...
  const DogImages = (item) => {
    return (
      <DogImgWrapper key={item.dogs.dogId}>
        <div
          className={selectedDog === item.dogs.dogId ? 'dogProfileCircle' : 'dogProfileCircle disabled'}
          onClick={() => {
    ...
    <CreateBoardWrapper>
      ...
      <BoardContentWrapper>
        ...
        <div className='dogImageWrapper'>
          {userData.dogs &&
            userData.dogs.map((item, idx) => <DogImages dogs={item} key={item.dogId} idx={idx}></DogImages>)}
        </div>
